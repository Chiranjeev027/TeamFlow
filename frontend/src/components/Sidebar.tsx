...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const currentUserStatus = currentOnlineUsers.find(u => u.userId === user?._id)?.status || 'online';
  ...
  const handleMenuClick = (item: typeof menuItems[0]) => {
    ...
    // Handle Dashboard sections (Dashboard, Projects, Analytics, Settings)
      ...
      if (onSectionChange) {
        // If we're already on Dashboard, just switch section
        onSectionChange(item.id);
      } else {
        // If we're on another page, navigate to Dashboard with section state
          ...
          <div className="border-t border-white/20 pt-4 mb-4">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex -space-x-2">
                    {teamMembers.slice(0, 3).map((member) => (
                      <div
                        key={member._id}
                        className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold text-white border-2 border-white ${currentOnlineUsers.some(u => u.userId === member._id) ? 'bg-green-500' : 'bg-gray-500'
                          }`}
                        title={`${member.name} (${currentOnlineUsers.some(u => u.userId === member._id) ? 'Online' : 'Offline'})`}
                      >
            ...
            <div className="flex items-center mb-3">
              <div className="relative group">
                <div className="w-10 h-10 rounded-full bg-green-500 flex items-center justify-center text-white font-semibold mr-3 cursor-pointer transition-transform hover:scale-105">
                ...
                {/* Status Menu */}
                <div className="absolute bottom-full left-0 mb-0 pb-2 w-40 hidden group-hover:block z-50 transform origin-bottom-left transition-all duration-200">
