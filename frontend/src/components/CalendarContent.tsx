...
const CalendarContent: React.FC = () => {
    ...
    const fetchEvents = useCallback(async () => {
        try {
            ...
            const end = endOfMonth(addDays(date, 42));
    ...
    const eventStyleGetter = (event: CalendarEvent) => {
        ...
        switch (event.type) {
            case 'task': backgroundColor = '#3b82f6'; break;
            case 'milestone': backgroundColor = '#10b981'; break;
            case 'team-event': backgroundColor = '#f59e0b'; break;
        }
        return {
            style: {
                backgroundColor,
    ...
    const handleEventDrop = async ({ event, start, end }: { event: CalendarEvent; start: Date; end: Date }) => {
        try {
            const token = localStorage.getItem('token');
            if (event.type === 'task') {
                await axios.patch(`/api/tasks/${event.id}`, { dueDate: start }, { headers: { Authorization: `Bearer ${token}` } });
            } else {
                await axios.put(`/api/team-events/${event.id}`, { startDate: start, endDate: end }, { headers: { Authorization: `Bearer ${token}` } });
            }
            fetchEvents();
        } catch (error) {
